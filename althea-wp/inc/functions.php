...
function althea_wp_page_builder_components($components)
{
...
function althea_wp_default_components($components)
{
...
function althea_wp_register_components($components = array())
{
    if (apply_filters('colibri_page_builder/installed', false)) {
        $components = althea_wp_page_builder_components($components);
    } else {
        $components = althea_wp_default_components($components);
    }

    return $components;
}

Hooks::prefixed_add_action('components', 'althea_wp_register_components');
Theme::load(array(
    'themeRelativePath' 		=> '',
 ...
 */
function althea_wp_assets()
{
    return althea_wp_theme()->getAssetsManager();
}
...
function althea_wp_register_sidebars()
{
    register_sidebar(array(
        'name' 			=> esc_html__('Blog sidebar widget area', 'althea-wp'),
    ...
    if (function_exists('\is_shop')) {
        register_sidebar(array(
            'name' 			=> esc_html__('Woo Commerce left sidebar widget area', 'althea-wp'),
            'id' 			=> 'althea-wp-ecommerce-left',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'before_title' 	=> '<h5 class="widgettitle">',
            'after_title' 	=> '</h5>',
            'after_widget' 	=> '</div>',
    ...
    if (!apply_filters('colibri_page_builder/installed', false)) {
        althea_wp_assets()
            ...
            ->registerStylesheet("althea-wp-theme", "/theme/theme.css")
    ...
    althea_wp_assets()->registerTemplateStyle('althea-wp-theme-extras', '/theme/extras.css', 'althea-wp-theme');
...
//blog options
    ...
    function althea_wp_show_post_meta_setting_filter($value)
    {
    ...
    add_filter('colibri_show_post_meta', 'althea_wp_show_post_meta_setting_filter');


    function althea_wp_posts_per_row_setting_filter($value)
    {
    ...
    add_filter('colibri_posts_per_row', 'althea_wp_posts_per_row_setting_filter');

    function althea_wp_archive_post_highlight_setting_filter($value)
    {
    ...
    add_filter('colibri_archive_post_highlight', 'althea_wp_archive_post_highlight_setting_filter');


    function althea_wp_blog_sidebar_enabled_setting_filter($value)
    {
    ...
    Hooks::prefixed_add_filter('blog_sidebar_enabled', 'althea_wp_blog_sidebar_enabled_setting_filter');
//add_filter( 'blog_sidebar_enabled', 'althea_wp_blog_sidebar_enabled_setting_filter' );

    function althea_wp_override_with_thumbnail_image($value)
    {
        global $post;
    ...
    add_filter('colibri_override_with_thumbnail_image', 'althea_wp_override_with_thumbnail_image');

    function althea_wp_print_archive_entry_class($class = "")
    {
        ...
        $postsPerRow = max(1, apply_filters('althea_wp_posts_per_row', $default));


        $classes[] = "h-col-sm-12 h-col-md-" . (12 / intval($postsPerRow));

        $classes = apply_filters('althea_wp_archive_entry_class', $classes);
    ...
    function althea_wp_print_masonry_col_class($echo = false)
    {

        global $wp_query;
        $index = $wp_query->current_post;
        $hasBigClass = (is_sticky() || ($index === 0 && apply_filters('althea_wp_archive_post_highlight', false)));
    ...
    Hooks::prefixed_add_filter('info_page_tabs', 'althea_wp_get_started_info_page_tab');

    function althea_wp_get_started_info_page_tab($tabs)
    {
    ...
    function althea_wp_theme_plugins($plugins)
    {
        $theme_plugins = array();
    ...
    Hooks::prefixed_add_filter('theme_plugins', 'althea_wp_theme_plugins');


    add_filter('http_request_host_is_external', 'althea_wp_allow_internal_host', 10, 3);
    function althea_wp_allow_internal_host($allow, $host, $url)
    {
    ...
    add_action('wp_ajax_althea_wp_front_set_predesign', function () {
        check_ajax_referer( 'althea_wp_front_set_predesign_nonce', 'nonce' );
    ...
    function althea_wp_override_main_row_class($classes)
    {
